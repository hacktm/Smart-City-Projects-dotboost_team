<?php
/**
 * Below lines are used in order to protect the file usage outside the admin panel 
 * Remove if you want to use the file standalone 
 */
session_start();
if (@$_SESSION['admin']['admin']->role !== 'admin') exit();
...
$VERSION='$Id: apcu.php 784 2014-02-18 23:28:33Z julian $';
...
<style><!--
body { background:#F9F9F9; font-size:100.01%; margin:0; padding:0; }
  ...
  echo
	...
	if ($list) {
      ...
      if(!$MYREQUEST['SEARCH'] || preg_match($MYREQUEST['SEARCH'], $entry[$fieldname]) != 0) {
		...
		echo '<tr class=tr-0><td class="center" colspan="7"><i>No data</i></td></tr>';
	}
	echo <<< EOB
		</tbody></table>
EOB;
